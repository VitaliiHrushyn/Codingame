 ...
 **/
class Player6 { /// rename to Player (version 6)
...
//System.err.println("equals: factory id= "+this.id+", source id= "+sourceFactory.id);
				if (this.arg3_production < INC_VALUE && this.arg2_numberOfCyborgs > 10 && myFactoriesAmount >= MIN_MY_FACTORIES_AMOUNT) {
					increase = true;
					return INC+this.id;
				} else {
				return "";
...
System.err.println("increase factory id, source id= "+details);
			...
			//		increase = true; //??????????????????????????????????????????????
					if (INC_VALUE <= sourceFactory.arg3_production) {
			...
			if (this.arg1_owner == ENEMY) {				
				...
				if (sourceFactory.arg2_numberOfCyborgs < cybCount && bombCount > 0) {
					bombCount--;
					return BOMB+details;
				}
				sourceFactory.removeSyborg(cybCount);
				if (INC_VALUE <= sourceFactory.arg3_production || myFactoriesAmount >= MIN_MY_FACTORIES_AMOUNT) {
				return MOVE+details+" "+cybCount;
				}
				return "";
			}
