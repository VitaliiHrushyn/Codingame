...
//            int unit0Rate = getTotalPointsRate(getPoint(myUnitList.get(0).unitCell, graph.allPointsList), graph.allPointsList);
...
//System.err.println("unit 1 rate = " + unit1Rate);
 ...
 System.err.println("line list = "+ graph.lineList.size());
...
//======================================================================================================================   
   ...
   static class Cell {
	   ...
	   int rate; // for new opportunities :)
		...
		public List<Cell> getAccessibleCellsList(List<Cell> cellList) {
		...
		void setConnectedPointsList(List<Cell> cellList, List<Line> lineList, List<Point> allPointsList) {
			...
			for (Cell cell : accessibleCells) {			
				if(!lineList.contains(line = new Line(startCell, cell))) {  
					lineList.add(line);					
					this.connectedPoints.add(getPoint(cell, allPointsList));
				}
			}			
		}
...
//System.err.println("getConnectedPR: start");
			...
			if (this.connectedPoints.size() == 0) {
System.err.println("conn rate = -1 (size 0) ");
				return -1;
			}
			int rate = 0;			
			for (Point point : this.connectedPoints) {								
				rate += point.position.rate;
System.err.println("Cell x, y, rate = "+point.position.x+" "+point.position.y+" "+point.position.rate+", point rate = "+rate);
			}
//System.err.println("conn rate = "+rate);
			return rate;
		}
	}
...
//=======================================================================================================================
		...
		static int getTotalPointsRate (Point startPoint, List<Point> allPointsList) {
			int totalRate = 0;
		
			int connectedPointsRate = startPoint.getConnectedPointsRate();
			if (connectedPointsRate != -1) {
			totalRate += connectedPointsRate;
//System.err.println("getTotPR: totalRate = " + totalRate);
			for (Point point : startPoint.connectedPoints) {
				point.getConnectedPointsRate();
				}			
			}
System.err.println("met: totalRate = " + totalRate);
			return totalRate;			
		}
