...
//System.err.println("unit 1 x y: rate = " + myUnitList.get(1).unitCell.x+" "+myUnitList.get(1).unitCell.y+": "+ unit1Rate);
           ...
           Map map1 = myUnitList.get(0).getPriorityMap();
           Set keys1 = map1.keySet();
           Cell goTo = null;
           int biggestRate = -1;
           for (Object object : keys1) {
        	   Cell cell = (Cell) object;
        	   int rate = (Integer) map1.get(cell);
        	   if(biggestRate < rate) {
        		   biggestRate = rate;
        		   goTo = cell;
        	   }		
           }
System.err.println("cell x y rate: "+goTo.x +" "+goTo.y+" "+biggestRate);
...
//======================================================================================================================   
   ...
   static class Cell {
	 ...
	 //  List<Cell> copyCellList;
		...
		public int getTotalCellRate (List<Cell> copyCellList) {
			int totalRate = this.rate;
			
			
			if (!copyCellList.contains(this)) {
				return totalRate;
			} else {
				for (Cell cell : this.connectedCells) {
					copyCellList.remove(this);
					totalRate += cell.rate;
//System.err.println("getTotRate - new iterate");
					totalRate += cell.getTotalCellRate(copyCellList);
				}
			}
			return totalRate;
		}
...
//==========================================================================================================================
   ...
   static class Unit {
	   ...
	   public Unit(int index, int unitX, int unitY, List<Cell> cellList) {
		...
		Map<Cell, Integer> getPriorityMap() {
			@SuppressWarnings("rawtypes")
			List<List> copyCellListsList = new ArrayList<>();
			for (int i = 0; i < this.unitCell.connectedCells.size(); i++) {
				copyCellListsList.add(new ArrayList<>(cellList));
			}
			Map<Cell, Integer> map = new HashMap<>();

			for (int i = 0; i < this.unitCell.connectedCells.size(); i++) {
				int rate = this.unitCell.connectedCells.get(i).getTotalCellRate(copyCellListsList.get(i));
				map.put(this.unitCell.connectedCells.get(i), rate);
			}
			return map;
		}
