...
//  System.err.println("availableA "+availableA);
//  System.err.println("availableB "+availableB);
//  System.err.println("availableC "+availableC);
//  System.err.println("availableD "+availableD);
//  System.err.println("availableE "+availableE);
...
System.err.println("have A "+haveA+", B "+haveB+", C "+haveC+", D "+haveD+", E "+haveE);   
	...
	//	if (totalStorage == 10) return "WAIT";    	
		if      (needA > haveA && availableA > 0) {moleculeType = "A"; haveA++;}
    		...
    		if (totalStorage == 9) goAhead = !goAhead;
   ...
   public static String doLaboratory() {
	    List<SampleData> myDatas = dataList.getMyDatas();
	   	...
	   	for (SampleData sd : myDatas) {
				...
				if (sd.willEnoughMolecules()) go = !go;
			}
	   	if (myDatas.size() == 1 || go) goAhead = !goAhead;
   ...
   static class DataList {
    	...
    	public int getBestRankData () {
    		...
    		if ( myExpertise < 4) return 1;
    		if ( myExpertise < 7) return 2;
    		return 3;    		
    	}
 ...
 @Deprecated
    ...
    static class SampleData {
		...
		public boolean willEnoughMolecules() {
			if (costA <= availableA &&
					costB <= availableB &&
						costC <= availableC &&
							costD <= availableD &&
								costE <= availableE) return true;
			return false;
		}
     }
